...
| **HumanLink** | **Agent 执行前如何留下不可伪造的人类授权记录** | **协议 + SDK** | **Agent 平台（toB）/ 个人用户（toC）** |
...
| **接入方式** | 本地 SDK + AI Gateway（如 OpenClaw）直连 | Verifier SDK 集成到服务端 |
...
**短期**：Verifier SDK 按验证次数向 Agent 平台收费。高风险操作（转账、删除、发布）每次验证付费。
...
║  ┌─ 本地 接入层 ─────────────┴────────────────────────────────────┐  ║
║  │                                                              │  ║
║  │  高风险操作由 Agent 平台拦截并发起认证                        │  ║
║  │  Agent 无法感知、干预认证流程                                 │  ║
║  │                                                              │  ║
║  │  [Copilot]  [Cursor]  [企业 Agent]  [Agent SaaS]            │  ║
║  │       │         │           │             │                  │  ║
║  │       └─────────┴───────────┴─────────────┘                  │  ║
║  │                         │                                    │  ║
║  │           HumanLink Verifier SDK（统一验证接口）              │  ║
║  │           · Agent 平台拦截高风险调用，生成 Challenge           │  ║
...
║  ┌─ 云端 接入层 ──────────────────────────────────────────────────┐ ║
║  │                                                               │ ║
║  │  Agent 跑在用户本机，AI Gateway 充当本地安全护栏               │ ║
║  │                                                               │ ║
║  │  [OpenClaw]  [本地 AI Gateway]  [消费者应用本地 SDK]           │ ║
║  │       └──────────────┴──────────────┘                         │ ║
║  │                         │                                     │ ║
║  │           HumanLink 本地 Verifier SDK                         │ ║
...
| 验证执行位置 | 云端 Verifier | 本地 Verifier SDK |
| 链上检查 | 必须 | 可选（降级为本地信任） |
...
无中心服务器时，审计记录写本地，格式与 toB 云端审计库兼容，可在需要时导出举证：
...
/*
toB 说明：平台内部数据库已维护 userId → deviceDID 映射时可不依赖此合约，直接从自有 DB 取。UserDeviceRegistry 主要为 toC 场景提供链上标准查询接口。
*/
...
### 面向 Agent 平台开发者：Verifier SDK

Verifier SDK 是 HumanLink 面向 Agent 平台的主要产品。
...
class HumanLinkVerifier:
    """
    核心验证器。Agent 平台引入此 SDK 即可生成 Challenge 并验证 Assertion。
    """
    ...
    toB 云端场景：优先从平台自有 DB 取，避免链上查询延迟。
    toC 本地场景：从链上 UserDeviceRegistry 取。
    """
    ...
    def create_challenge(
        ...
        action_params: 平台自己解析到的真实操作参数（非 Agent 描述）
        actionHash 在内部计算: sha256(action + sorted(params) + nonce)
        origin 自动填入 self.origin
        display 由平台填写，将显示在用户的 HumanLink 设备上

        返回 Challenge 对象，包含 nonce（用于后续验证匹配）
        """
...
前提：
平台必须维护用户绑定表，`userId` 由平台自身 Auth 层（IAM/Gateway 解析 Agent Token）可信传入，HumanLink 不处理 Agent 身份认证。
...
```python
# Agent 平台集成 HumanLink（以 Copilot 类平台为例）
...
async def agent_transfer(req: AgentTransferRequest, session: UserSession):
    # 1. 平台自己的风险拦截逻辑（与 Agent 无关）
    # userId 由平台 Auth 层从 Agent Token 解析，HumanLink 不处理此步骤
    if req.amount > 100 or req.to not in session.user.trusted_list:

        # 2. SDK 内部自动查用户绑定 DID 并嵌入 Challenge
        challenge = verifier.create_challenge(
        ...
        await audit_log.write(
            ...
            params=req.dict(),
            assertion_id=assertion["id"],
            device_did=result.device_did,    # 已验证与用户绑定 DID 一致
            timestamp=assertion["created"]
        )
...
│   │       └── crypto_utils.cpp / .h  ← SHA256
│   │
│   ├── core/                          ← PC SDK 守护进程 (Python, 跨平台)
│   │   ├── hardware/
...
│   │   │   ├── server.py              ← FastAPI
│   │   │   └── routes.py              ← 路由
│   │   │   └── ws_client.py           ← 云端场景: WebSocket 接收云端 Challenge
│   │   └── db/
│   │       └── store.py               ← SQLite
│   │
│   └── apps/
│       └── toB_platform/              ← 云端 Demo（WebSocket 模拟云端）
│       └── toC_openclaw/              ← 本地 + OpenClaw Demo
│
├── config.yaml
├── requirements.txt
└── README.md
...
| 设备信任分级 | attestation + trust_policy | Verifier SDK |
| 可举证（toB云端） | 审计库 + 链上，平台无需 HumanLink 配合即可举证 | 平台层 |
| 可举证（toC本地） | 本地审计记录由设备私钥签名。链上 UserDeviceRegistry，用户可自证 | 本地 + 链上 |

---

*文档版本 v2.1 | HumanLink Protocol v0.3*
