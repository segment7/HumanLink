...
> 注：关于本地化技术栈和文件结构参见 demo.md ，使用指南（本地跑通方法）参见 DEMO_GUIDE.md
...
## 二、业务流程图
基于HumanLink通用协议设计，HumanLink 支持两种主要实现路径：

### 本地集成路径（AI Gateway 集成）
...
┌────────────────────────── 用户本机 ──────────────────────────────┐
│                                                                  │
│  [Agent 进程] ──▶ [OpenClaw] ──▶ [HumanLink SDK] ──▶ [U盾设备]   │
│  工具调用         策略拦截       生成Challenge      物理确认       │
│                                本地验证                          │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
...
## 五、HumanLink 协议设计（通用基础设施）

HumanLink 协议是通用的人类在场授权基础设施，不区分本地或云端部署。协议核心、数据结构、链上基础设施在所有场景下保持一致，仅在具体实现路径上有所差异。

### 协议核心：HumanLink Assertion Protocol v0.3

### Challenge 生成机制：Requestor-Initiated

HumanLink 的 Challenge **只能由承担责任的请求方生成**（云端为 Agent 平台，本地为 AI Gateway）。Agent 不参与 Challenge 的生成、传递或修改。

```
请求方               承担责任的验证方                    HumanLink Issuer
(Agent)              (平台/AI Gateway)                      │
  │                          │                              │
  │  1. 操作请求              │                              │
  │  (transfer/delete/...)   │                              │
...
**Agent 完全不接触 Challenge**。Agent 调用验证方 API，验证方自行决定是否需要认证、自行生成 Challenge、自行与 HumanLink Issuer 通信。Agent 只知道"操作成功"或"操作被拒绝"，不知道认证流程的存在，因此也无法绕过或伪造。

**`actionHash` 由验证方基于真实操作参数计算**。即使 Agent 描述的是"更新偏好"，但验证方解析到的实际调用是 `deleteAccount(id=12345)`——Challenge 里写的是后者，用户屏幕上显示的也是后者。

**`display` 由验证方填写**。用户在 HumanLink 设备上看到的操作摘要来自承担责任的验证方，不来自 Agent 本身。验证方对自己拦截到的业务语义的描述是可信的。
...
*文档版本 v2.3 | HumanLink Protocol v0.3*
